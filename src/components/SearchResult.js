    ...
    useEffect (() => {
        const fetchFlowerInfo = async () => {
            try {
              ...
              const response = await axios.get(`https://fdy-backend-server-wgqg.onrender.com/flowers?flowername=${label}`);
...
return (
    <div>
        ...
        <div style={{ backgroundColor: 'white', padding: '10px', borderRadius: '8px', display: 'flex',flexDirection: 'column' }}>
                <h2 style={{ color: 'darkgreen', fontSize: '20px', alignSelf: 'flex-start', marginBottom:
                '10px' }}>
